...
test('Validating Name expected behaviour', async ({page}) => {
  ...
  await page.keyboard.press('Tab');     
  await expect(page.getByText('Please enter only characters.')).not.toBeVisible();
  await expect(page.getByText('Must be atleast 3 characters long.')).not.toBeVisible();
  await expect(page.getByText('Must not be greater than 20 characters long.')).not.toBeVisible();
...
test('Validating Email expected behaviour', async ({page}) => {
  ...
  await expect(page.getByText('Must be atleast 3 characters long.')).not.toBeVisible();
  await expect(page.getByText('Must be valid email.')).not.toBeVisible();
});
...
test('Validating Contact Number expected behaviour', async ({page}) => {
  ...
  await page.keyboard.press('Tab');     
  await expect(page.getByText('Contact Number is required.')).not.toBeVisible();
  await expect(page.getByText('Invalid Phone Number.')).not.toBeVisible();
  await expect(page.getByText('Must be 10 digits.')).not.toBeVisible();
  await expect(page.getByText('Must not be more than 10 digits.')).not.toBeVisible();
});
...
test('Validating Message expected behaviour', async ({page}) => {
  ...
  await expect(page.getByText('Must be atleast 5 characters long.')).not.toBeVisible();
  await expect(page.getByText('Must not be more than 50 characters long.')).not.toBeVisible();
});
